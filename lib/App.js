	...
	componentWillMount() {
		...
		.then( data => {
			...
			data.forecast.simpleforecast.forecastday.forEach ( (day, index) => {
				if (index < 7) {
					tenDayData.iconArray.push(day.icon_url);
					tenDayData.hiArray.push(day.high.fahrenheit);
					tenDayData.loArray.push(day.low.fahrenheit);
				}
			})

			data.forecast.txt_forecast.forecastday.forEach ( (day, index) => {
				if (index % 2 === 0 && index < 13) {
					tenDayData.dayArray.push(day.title);
				}
			})
			...
			this.setState( { currentWeatherData, tenHourData, tenDayData } )
		})
