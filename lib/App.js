	...
	componentWillMount() {
		...
		.then( data => {
			...
			data.forecast.simpleforecast.forecastday.forEach ( (day, index) => {
				if (index < 7) {
					sevenDayData.iconArray.push(day.icon_url);
					sevenDayData.hiArray.push(day.high.fahrenheit);
					sevenDayData.loArray.push(day.low.fahrenheit);
				}
			})

			data.forecast.txt_forecast.forecastday.forEach ( (day, index) => {
				if (index % 2 === 0 && index < 13) {
					sevenDayData.dayArray.push(day.title);
				}
			})
